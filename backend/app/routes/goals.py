...
def get_pending_goals(db: Session = Depends(get_db),
...
def submit_goal(goal_id: int, db: Session = Depends(get_db),
                current_user: User = Depends(get_current_user)):
...
def approve_goal(goal_id: int, payload: GoalFeedback, db: Session = Depends(get_db),
                 current_user: User = Depends(get_current_manager)):
...
def reject_goal(goal_id: int, payload: GoalFeedback, db: Session = Depends(get_db),
                current_user: User = Depends(get_current_manager)):
    goal = db.query(Goal).filter(Goal.id == goal_id).first()
    if not goal: raise HTTPException(status_code=404, detail="Goal not found")
