...
def create_goal(goal: GoalCreate, db: Session = Depends(get_shard2_db),
...
def get_pending_goals(db: Session = Depends(get_shard2_db),
                      current_user: User = Depends(get_current_manager)):
    return db.query(Goal).filter(Goal.status == GoalStatusEnum.SUBMITTED).all()

@router.get("/", response_model=List[GoalResponse])
def list_goals(status: Optional[str] = None, db: Session = Depends(get_shard2_db),
               current_user: User = Depends(get_current_user)):
    query = db.query(Goal)
    if status:
...
def get_employee_goals(employee_id: int, db: Session = Depends(get_shard2_db),
                       current_user: User = Depends(get_current_user)):
    return db.query(Goal).filter(Goal.employee_id == employee_id).all()

@router.patch("/{goal_id}/submit", response_model=GoalResponse)
def submit_goal(goal_id: int, db: Session = Depends(get_shard2_db),
                current_user: User = Depends(get_current_user)):
...
def approve_goal(goal_id: int, payload: GoalFeedback, db: Session = Depends(get_shard2_db),
                 current_user: User = Depends(get_current_manager)):
...
def reject_goal(goal_id: int, payload: GoalFeedback, db: Session = Depends(get_shard2_db),
                current_user: User = Depends(get_current_manager)):
    goal = db.query(Goal).filter(Goal.id == goal_id).first()
    if not goal: raise HTTPException(status_code=404, detail="Goal not found")
