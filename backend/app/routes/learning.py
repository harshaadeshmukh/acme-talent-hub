...
def create_certificate(cert: TrainingRecordCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def verify_certificate(cert_id: int, verify_data: CertificateVerifyRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    if current_user.role != RoleEnum.MANAGER:
...
def update_certificate(cert_id: int, cert_data: TrainingRecordCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    db_cert = db.query(TrainingRecord).filter(TrainingRecord.id == cert_id).first()
    if not db_cert:
        raise HTTPException(status_code=404, detail="Certificate not found")
...
def update_goal(goal_id: int, goal_data: GoalCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def update_goal_progress(goal_id: int, progress: GoalProgressUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    db_goal = db.query(Goal).filter(Goal.id == goal_id).first()
    if not db_goal:
        raise HTTPException(status_code=404, detail="Goal not found")
...
def add_goal_feedback(goal_id: int, feedback: GoalFeedback, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    if current_user.role != RoleEnum.MANAGER:
